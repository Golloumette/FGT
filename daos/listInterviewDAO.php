...
function lastInsert (PDO $pdo) :array{
        ...
        try{
            $sql="SELECT *FROM studenttalent ORDER BY id DESC LIMIT 5";
             $sql2="SELECT studenttalent.prenom,studenttalent.age,ecole.nom,campus_ville.ville_campus,cursus.nom_cursus,studenttalent.talent,studenttalent.photo FROM studenttalent 
             INNER JOIN cursus ON studenttalent.cursus_ID = cursus.id
             INNER JOIN campus_ville ON studenttalent.cursus_id =cursus.id
